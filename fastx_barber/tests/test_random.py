...
def test_make_random_string():
    random_string = random.make_random_string(FAKE_SEQ_LEN, TEST_ALPHABET)
    assert FAKE_SEQ_LEN == len(random_string)
    assert all([c in TEST_ALPHABET for c in random_string])


def assert_fasta_record(
    record: seqio.SimpleFastaRecord, expected_name: str = FAKE_NAME
) -> None:
...
def assert_fastq_record(
    ...
    assert FAKE_SEQ_LEN == len(record[2])
    assert all([c in random.QUAL_ALPHABET for c in record[2]])


def test_make_fastq_record():
    assert_fastq_record(random.make_fastq_record(FAKE_NAME, FAKE_SEQ_LEN))


def test_make_fastq_file():
    fastq = random.make_fastq_file(FAKE_N_RECORDS, FAKE_SEQ_LEN)
    for i in range(len(fastq)):
...
def test_write_tmp_fasta_file():
    fpath, dpath = random.write_tmp_fasta_file(FAKE_N_RECORDS, FAKE_SEQ_LEN)
    assert_fastx_file(fpath, const.FastxFormats.FASTA)
    zpath, _ = random.write_tmp_fasta_file(
        FAKE_N_RECORDS, FAKE_SEQ_LEN, dpath, gzipped=True
    )
...
def test_write_tmp_fastq_file():
    fpath, dpath = random.write_tmp_fastq_file(FAKE_N_RECORDS, FAKE_SEQ_LEN)
...
def test_write_tmp_fastx_file():
    ...
    assert_fastx_file(zpath, const.FastxFormats.FASTA)
    fpath, _ = random.write_tmp_fastq_file(FAKE_N_RECORDS, FAKE_SEQ_LEN, dpath)
    assert_fastx_file(fpath, const.FastxFormats.FASTQ)
    zpath, _ = random.write_tmp_fastq_file(
        FAKE_N_RECORDS, FAKE_SEQ_LEN, dpath, gzipped=True
    )
